...
export class Game {
  ...
  updateHeroes(dt){
    ...
    for(const unit of livingUnits){
      unit.status.slow=Math.max(0,(unit.status.slow||0)-dt);unit.status.riposte=Math.max(0,(unit.status.riposte||0)-dt);unit.status.haste=Math.max(0,(unit.status.haste||0)-dt);unit.outreadCooldown=Math.max(0,(unit.outreadCooldown||0)-dt);unit.visualHitPulse=Math.max(0,(unit.visualHitPulse||0)-dt*5.8);
