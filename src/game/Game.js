...
export class Game {
  ...
  damageEnemy(enemy,amount,source,critical=false,flags={}){
    ...
    let remaining=amount,absorbed=0;if(enemy.shield>0){absorbed=Math.min(enemy.shield,remaining);enemy.shield-=absorbed;remaining-=absorbed}enemy.hp-=remaining;if(enemy.wardOnHit>0&&enemy.alive&&enemy.wardHitCooldown<=0){enemy.shield+=enemy.maxHp*enemy.wardOnHit;enemy.wardHitCooldown=.72}
