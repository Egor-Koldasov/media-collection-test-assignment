...
export class Game {
  ...
  spawnEnemy(type=chooseEnemyType(this.elapsed/TOTAL_TIME),position=null,forcedAffixes=null){
    ...
    const strength=(.72+progress*.75+Math.pow(progress,2)*1.75)*this.difficulty.health;const damageCurve=(.28+progress*.22+Math.pow(progress,2)*.75)*this.difficulty.damage;const angle=Math.random()*Math.PI*2;const radiusX=Math.max(11.5,Math.abs(this.camera.right)*.92);const radiusY=7.7;
