...
export class Game {
  ...
  resize(){if(this.headless)return;const width=Math.max(1,this.stage.clientWidth),height=Math.max(1,this.stage.clientHeight),pixelRatio=Math.min(window.devicePixelRatio,1.75);this.renderer.setPixelRatio(pixelRatio);this.renderer.setSize(width,height,false);const aspect=width/height,vertical=18.4;this.camera.top=vertical/2;this.camera.bottom=-vertical/2;this.camera.left=-vertical*aspect/2;this.camera.right=vertical*aspect/2;this.applyCameraRig();this.hud?.resize(width,height,pixelRatio)}
  ...
  setModalPaused(value){this.modalPaused=value;this.hud&&(this.hud.dirty=true)}
  ...
  render(time){if(this.headless)return;this.arena.update(time,this.elapsed/TOTAL_TIME);this.units.forEach((unit)=>{if(unit.alive)updateEntityVisual(unit,unit.id===this.selectedId,time)});this.enemies.forEach((enemy)=>{if(enemy.alive)updateEntityVisual(enemy,false,time)});this.treasures.forEach((treasure)=>updateTreasureVisual(treasure,time));let shakeX=0,shakeY=0;if(this.cameraShake>0){this.cameraShake=Math.max(0,this.cameraShake-.018);shakeX=(Math.random()-.5)*this.cameraShake;shakeY=(Math.random()-.5)*this.cameraShake}this.camera.position.x=this.cameraRig.x+shakeX;this.camera.position.y=this.cameraRig.y+shakeY;this.renderer.render(this.scene,this.camera);const renderInfo=this.renderer.info.render;this.hud?.render(this.snapshot(),time,{audioEnabled:this.audio.enabled,cameraX:this.cameraRig.x,cameraY:this.cameraRig.y,cameraZoom:this.cameraRig.zoom,maxEnemies:this.maxEnemies,calls:renderInfo.calls,triangles:renderInfo.triangles})}
}
