...
export class Game {
  ...
  damageEnemy(enemy,amount,source,critical=false,flags={}){
    ...
    if(source?.mods?.lifesteal)this.heal(source,remaining*source.mods.lifesteal,false);if(!flags.quiet){this.queueCombatNumber(enemy,remaining,{kind:critical?'critical':'damage',prefix:critical?'✧ ':''});this.audio.play('combatImpact',{gain:critical?1.15:1})}
