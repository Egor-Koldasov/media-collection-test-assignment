...
export class Game {
  ...
  loop(){this.animationFrame=requestAnimationFrame(()=>this.loop());const dt=Math.min(.05,this.clock.getDelta());if(this.running&&!this.paused&&!this.modalPaused&&!this.ended)this.update(dt);else if(this.ended&&(this.units.some((unit)=>!unit.alive&&!unit.deathVisualDone)||this.enemies.some((enemy)=>!enemy.alive&&!enemy.deathVisualDone))){this.updateDeaths(dt);this.cleanup()}this.render(performance.now())}
  ...
  updateHeroes(dt){
    ...
    for(const unit of livingUnits){
      unit.status.slow=Math.max(0,(unit.status.slow||0)-dt);unit.status.riposte=Math.max(0,(unit.status.riposte||0)-dt);unit.status.haste=Math.max(0,(unit.status.haste||0)-dt);unit.outreadCooldown=Math.max(0,(unit.outreadCooldown||0)-dt);unit.visualHitPulse=Math.max(0,(unit.visualHitPulse||0)-dt*5.8);unit.visualCastPulse=(unit.visualCastPulse||0)*Math.exp(-dt*5.7);
      ...
      unit.ap-=cost;ability.cooldownLeft=ability.cooldown*unit.mods.cooldown;unit.abilityCursor=(unit.abilityCursor+1)%unit.abilities.length;unit.casts+=1;unit.lastAbilityKind=ability.kind;unit.lastAbilityCategory=ability.category||ability.kind;unit.visualCastPulse=1;
  ...
  cast(unit,ability,slot,powerScale=1,isEcho=false){
    ...
    if(ability.kind==='heal'||ability.kind==='ward'||ability.kind==='transfusion'){
      ...
      if(ability.kind==='ward'){const amount=ability.power*unit.mods.ward*powerScale;ally.shield+=amount;this.effects.push(createBurst(this.scene,ally.x,ally.y,ability.color,.7,'heal'));this.queueCombatNumber(ally,amount,{kind:'ward',prefix:'+',suffix:' WARD'})}
  ...
  queueCombatNumber(target,amount,{kind='damage',prefix='',suffix=''}={}){
    if(this.headless||!target||amount<=.2)return;const key=`${kind}:${target.id}:${prefix}:${suffix}`,existing=this.combatTextQueue.get(key);if(existing){existing.amount+=amount;existing.hits+=1;existing.x=target.x;existing.y=target.y;return}const lane=((target.id+this.combatTextSequence++)%3)-1;this.combatTextQueue.set(key,{amount,hits:1,x:target.x,y:target.y,kind,prefix,suffix,lane,timer:.09});
  }

  updateCombatText(dt){
    for(const [key,item] of this.combatTextQueue){item.timer-=dt;if(item.timer>0)continue;const label=`${item.prefix}${Math.round(item.amount)}${item.suffix}`;this.effects.push(createFloatingText(this.scene,label,item.x,item.y,item.kind==='critical'?'#f0b85f':item.kind==='wound'?'#df5555':item.kind==='heal'?'#e3d89f':item.kind==='ward'?'#9aa8d5':'#dfd4bf',item.kind,item.lane));this.combatTextQueue.delete(key)}
  }
  ...
  damageEnemy(enemy,amount,source,critical=false,flags={}){
    ...
    enemy.visualHitPulse=1;enemy.visualHitDirection=source?Math.sign(enemy.x-source.x):0;let remaining=amount,absorbed=0;if(enemy.shield>0){absorbed=Math.min(enemy.shield,remaining);enemy.shield-=absorbed;remaining-=absorbed}enemy.hp-=remaining;if(enemy.wardOnHit>0&&enemy.alive&&enemy.wardHitCooldown<=0){enemy.shield+=enemy.maxHp*enemy.wardOnHit;enemy.wardHitCooldown=.72}
    if(source?.mods?.lifesteal)this.heal(source,remaining*source.mods.lifesteal,false);if(!flags.quiet){if(absorbed>.2)this.queueCombatNumber(enemy,absorbed,{kind:'ward',prefix:'−',suffix:' WARD'});this.queueCombatNumber(enemy,remaining,{kind:critical?'critical':'damage',prefix:critical?'✧ ':''});this.audio.play('combatImpact',{gain:critical?1.15:1})}
  ...
  updateDeaths(dt){
    if(this.headless)return;const visualTime=performance.now();for(const enemy of this.enemies){if(enemy.alive||enemy.deathVisualDone||!enemy.mesh)continue;enemy.deathAge=(enemy.deathAge||0)+dt;enemy.visualHitPulse=Math.max(0,1-enemy.deathAge*6.4);updateEntityVisual(enemy,false,visualTime);const t=Math.min(1,enemy.deathAge/.28),settle=1-Math.pow(1-t,3),data=enemy.mesh.userData,body=data.body,spriteUniforms=data.sprite?.userData?.spriteUniforms;[data.hp,data.ap,data.statusRing,data.shieldRing,data.eliteRing,data.crown].forEach((object)=>{if(object)object.visible=false});if(body){body.scale.y=enemy.deathBodyScaleY*(1+enemy.visualHitPulse*.028)*(1-settle*.82);body.position.y=enemy.deathBodyY-settle*.56}if(spriteUniforms)spriteUniforms.uOpacity.value=(data.baseOpacity||1)*(1-settle);if(data.shadow){data.shadow.scale.x=1+settle*.22;data.shadow.material.opacity=.4*(1-settle*.35)}if(t>=1){enemy.mesh.visible=false;enemy.deathVisualDone=true}}
    for(const unit of this.units){if(unit.alive||unit.deathVisualDone||!unit.mesh)continue;unit.deathVisualAge=(unit.deathVisualAge||0)+dt;unit.visualHitPulse=Math.max(0,1-unit.deathVisualAge*5.8);updateEntityVisual(unit,false,visualTime);const t=Math.min(1,unit.deathVisualAge/.18),settle=1-Math.pow(1-t,3),data=unit.mesh.userData,body=data.body,spriteUniforms=data.sprite?.userData?.spriteUniforms;if(unit.deathVisualAge<=dt+.001)[data.hp,data.ap,data.shield,data.selectedWash,data.selectedBracket,data.selectedSigil,data.riteHalo,data.orbitKnives].forEach((object)=>{if(object)object.visible=false});if(body){body.scale.y=(unit.deathBodyScaleY||1)*(1+unit.visualHitPulse*.025)*(1-settle*.58);body.position.y=(unit.deathBodyY||0)-settle*.32}if(spriteUniforms)spriteUniforms.uOpacity.value=1-settle;if(data.shadow)data.shadow.material.opacity=.43*(1-settle);if(data.oathLight)data.oathLight.userData.radialUniforms.uOpacity.value=.06*(1-settle);if(t>=1){unit.mesh.visible=false;unit.deathVisualDone=true}}
  }

  updateEnemies(dt){
    const livingUnits=this.units.filter((unit)=>unit.alive);if(!livingUnits.length)return;const now=performance.now();
    for(const enemy of this.enemies){
      if(!enemy.alive)continue;enemy.visualBirthAge=Math.min(enemy.visualBirthDuration??.68,(enemy.visualBirthAge??.68)+dt);enemy.visualHitPulse=Math.max(0,(enemy.visualHitPulse||0)-dt*6.4);enemy.attackVisualRecovery=Math.max(0,(enemy.attackVisualRecovery||0)-dt);if(enemy.visualBirthAge<(enemy.visualBirthDuration??.68)){enemy.vx=0;enemy.vy=0;enemy.aiState='Rising from the grave';continue}this.tickEnemyStatuses(enemy,dt);if(!enemy.alive)continue;
  ...
  enemySupportCast(enemy){
    enemy.ap-=enemy.attackCost;enemy.auraTimer=4.2+Math.random()*1.2;const recovery=enemy.mesh?.userData?.attackProfile?.recover??.26;enemy.attackVisualRecovery=recovery;enemy.attackVisualRecoveryDuration=recovery;
  ...
  damageHero(unit,amount,source){
    ...
    if(distance(unit,source)<1.9&&unit.mods.closeGuard)amount*=Math.max(.7,1-unit.mods.closeGuard);unit.visualHitPulse=1;unit.visualHitDirection=source?Math.sign(unit.x-source.x):0;
    ...
    const absorbed=Math.max(0,amount-remaining);if(absorbed>.2)this.queueCombatNumber(unit,absorbed,{kind:'ward',prefix:'−',suffix:' WARD'});if(remaining>.2)this.queueCombatNumber(unit,remaining,{kind:'wound',prefix:'−'});this.effects.push(createHitSpark(this.scene,unit.x,unit.y,0xd94a4e,1));this.cameraShake=Math.max(this.cameraShake,.045);this.audio.play(remaining>0?'heroWound':'combatImpact',{gain:remaining>0?1:.75});
    if(unit.hp<=0){unit.alive=false;unit.hp=0;unit.visualCastPulse=0;unit.deathVisualAge=0;unit.deathVisualDone=this.headless;unit.deathBodyScaleY=unit.mesh?.userData?.body?.scale?.y||1;unit.deathBodyY=unit.mesh?.userData?.body?.position?.y||0;if(this.headless)unit.mesh.visible=false;this.feed(`<strong>${unit.name}</strong> is taken by the grave.`,true);this.effects.push(createParticleBurst(this.scene,unit.x,unit.y,0x8f2b35,20,1.5));const survivors=this.units.filter((ally)=>ally.alive);survivors.forEach((ally)=>{ally.shield+=ally.maxHp*.07;ally.ap=Math.min(ally.maxAp,ally.ap+ally.maxAp*.18);this.effects.push(createBurst(this.scene,ally.x,ally.y,0xd5a25c,.55))});if(survivors.length)this.feed('The remaining oaths tighten around the empty name.');this.events.onRoster?.(this.snapshot());const next=survivors[0];if(next)this.selectedId=next.id}
  }
  ...
  end(victory){if(this.ended)return;this.units.forEach((unit)=>unit.visualCastPulse=0);this.ended=true;this.running=false;this.audio.play(victory?'ritualBell':'heroWound',{gain:1.25,rate:victory ? 0.92 : 0.82});const result={victory,kills:this.killCount,upgrades:this.appliedUpgradeIds.size,units:this.units.filter((unit)=>unit.alive).length,elapsed:this.elapsed,difficulty:this.difficultyLevel,lawIds:this.graveLaws.map((law)=>law.id)};recordRun(this.chronicle,result);this.events.onDiscovery?.(this.snapshot());this.events.onEnd?.({...result,chronicle:this.chronicle})}
  snapshot(){
