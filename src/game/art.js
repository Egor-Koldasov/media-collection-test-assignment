...
function animatedAtlasPlane(path,columns,rows,frameCell,opacity=1,grade=null){
  ...
  const material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,side:THREE.DoubleSide,vertexShader:`
    ...
    void main(){
      ...
      p.x+=uRelease*(.025+.028*shoulders);p.y-=uRelease*hem*.022;
      p.x-=uHit*(uv.y-.22)*.035;
...
export function updateEntityVisual(entity,selected=false,time=0){
  ...
  if(spriteUniforms){spriteUniforms.uTime.value=time*.001;spriteUniforms.uMotion.value=motion;spriteUniforms.uAction.value=action;spriteUniforms.uRelease.value=recovery;spriteUniforms.uHit.value=hitPulse;spriteUniforms.uStride.value=stride;spriteUniforms.uLean.value=lean}
  if(entity.kind==='hero'){
