...
export function createEnemyIntent(scene,enemy,target,color,duration,kind='melee'){
  const ranged=kind==='ranged',cleave=kind==='cleave',kindIndex=cleave?2:ranged?1:0,strikeRadius=cleave?enemy.range+1:0,uniforms={uColor:{value:new THREE.Color(color)},uSize:{value:new THREE.Vector2(1,1)},uSpan:{value:1},uRadius:{value:strikeRadius},uProgress:{value:0},uKind:{value:kindIndex},uSeed:{value:(enemy.id%17)*.73+kindIndex*2.1}},material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,depthTest:true,side:THREE.DoubleSide,vertexShader:`
    varying vec2 vUv;
  ...
  `,fragmentShader:`
    ...
    void main(){
      ...
      if(uKind<.5){
        ...
        vec2 ep=vec2(p.x,p.y/.76);float radius=length(ep),angle=fract((atan(ep.y,ep.x)+3.14159265)/6.2831853),segments=.36+.64*step(.22,fract(angle*12.+uSeed*.17)),sweep=step(angle,.12+uProgress*.88);float outer=stroke(radius-uRadius,.026),inner=stroke(radius-uRadius*.79,.018),closing=stroke(radius-uRadius*(1.09-uProgress*.09),.012),ticks=stroke(radius-uRadius*.9,.055)*smoothstep(.7,.96,abs(sin(angle*3.14159265*16.)));float spoke=lineStroke(ep,vec2(uRadius*.24,0.),vec2(uRadius*.92,0.),.014),fang=max(lineStroke(ep,vec2(uRadius*.82,0.),vec2(uRadius*.65,.16*uRadius),.014),lineStroke(ep,vec2(uRadius*.82,0.),vec2(uRadius*.65,-.16*uRadius),.014));alpha=max(alpha,outer*segments*(.32+finale*.56));alpha=max(alpha,inner*sweep*(.2+finale*.3));alpha=max(alpha,closing*(.14+finale*.2));alpha=max(alpha,ticks*(.13+finale*.29));alpha=max(alpha,spoke*(.2+finale*.5));alpha=max(alpha,fang*finale*.7);soft=max(soft,stroke(radius-uRadius,.12)*(.06+finale*.06));
      }
  ...
  const place=()=>{const dx=target.x-enemy.x,dy=target.y-enemy.y,length=Math.max(.1,Math.hypot(dx,dy)),angle=Math.atan2(dy,dx);if(cleave){const extent=strikeRadius*2.32;quad.position.set(enemy.x,enemy.y,.045);quad.scale.set(extent,extent,1);uniforms.uSize.value.set(extent,extent);uniforms.uSpan.value=0}else{const height=ranged?1.45:1.18,width=length+(ranged?1.5:1.3);quad.position.set((enemy.x+target.x)/2,(enemy.y+target.y)/2,.045);quad.scale.set(width,height,1);uniforms.uSize.value.set(width,height);uniforms.uSpan.value=length}quad.rotation.z=angle};place();return{mesh:quad,drawCalls:1,kind,cancel(){cancelled=true},update(dt){if(cancelled)return true;age+=dt;uniforms.uProgress.value=Math.min(1,age/duration);place();return age>=duration+.06},destroy(){scene.remove(quad);disposeObject(quad)}};
}
