  ...
  `,fragmentShader:`
    ...
    void main(){
      ...
      if(uKind<.5){
        ...
        vec2 ep=vec2(p.x,p.y/.76);float radius=length(ep),angle=fract((atan(ep.y,ep.x)+3.14159265)/6.2831853),segments=.36+.64*step(.22,fract(angle*12.+uSeed*.17)),sweep=step(angle,.12+uProgress*.88);float outer=stroke(radius-uRadius,.026),inner=stroke(radius-uRadius*.79,.018),closing=stroke(radius-uRadius*(1.09-uProgress*.09),.012),ticks=stroke(radius-uRadius*.9,.055)*smoothstep(.7,.96,abs(sin(angle*3.14159265*16.)));float ca=cos(uAngle),sa=sin(uAngle);vec2 aim=vec2(ca*p.x+sa*p.y,-sa*p.x+ca*p.y);float spoke=lineStroke(aim,vec2(uRadius*.24,0.),vec2(uRadius*.92,0.),.014),fang=max(lineStroke(aim,vec2(uRadius*.82,0.),vec2(uRadius*.65,.16*uRadius),.014),lineStroke(aim,vec2(uRadius*.82,0.),vec2(uRadius*.65,-.16*uRadius),.014));alpha=max(alpha,outer*segments*(.32+finale*.56));alpha=max(alpha,inner*sweep*(.2+finale*.3));alpha=max(alpha,closing*(.14+finale*.2));alpha=max(alpha,ticks*(.13+finale*.29));alpha=max(alpha,spoke*(.2+finale*.5));alpha=max(alpha,fang*finale*.7);soft=max(soft,stroke(radius-uRadius,.12)*(.06+finale*.06));
      }
