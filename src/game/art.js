...
const enemyAttackProfiles={
  thrall:{pull:.07,lunge:.13,crouch:.035,twist:.052,recover:.2},
  hound:{pull:.12,lunge:.24,crouch:.105,twist:.028,recover:.18},
  pikeman:{pull:.17,lunge:.21,crouch:.028,twist:.026,recover:.24},
  bowman:{pull:.1,lunge:.025,crouch:.018,twist:.072,recover:.22},
  harvester:{pull:.12,lunge:.17,crouch:.082,twist:.115,recover:.3},
  graveguard:{pull:.08,lunge:.12,crouch:.058,twist:.045,recover:.26},
  cantor:{pull:.045,lunge:.02,crouch:-.022,twist:.056,recover:.26},
  standard:{pull:.035,lunge:.015,crouch:-.018,twist:.032,recover:.3},
  wraith:{pull:.1,lunge:.28,crouch:-.045,twist:.045,recover:.22},
  bishop:{pull:.045,lunge:.012,crouch:-.028,twist:.042,recover:.3},
  ossuary:{pull:.06,lunge:.1,crouch:.065,twist:.026,recover:.32},
  giant:{pull:.16,lunge:.21,crouch:.12,twist:.082,recover:.38}
};
const heroCastProfiles={
  melee:{travel:.105,lift:.008,spread:.018,rise:.018,twist:-.05,halo:.14},
  ranged:{travel:-.055,lift:-.012,spread:.034,rise:-.016,twist:.026,halo:.11},
  support:{travel:0,lift:.068,spread:-.026,rise:.06,twist:.006,halo:.3},
  zone:{travel:0,lift:-.042,spread:.07,rise:-.052,twist:-.012,halo:.23},
  control:{travel:-.035,lift:-.024,spread:.042,rise:-.026,twist:.045,halo:.18}
};
const heroCastKindOverrides={
  dash:{travel:.16,lift:.018,spread:-.012,rise:.018,twist:-.038},
  pounce:{travel:.22,lift:.09,spread:-.02,rise:.04,twist:-.055},
  cleave:{travel:.025,spread:.035,twist:-.06},reap:{travel:.035,spread:.03,twist:-.075},
  bash:{travel:.055,spread:.035,rise:-.025,twist:.018},riposte:{travel:-.17,spread:.04,rise:-.018,twist:.055},
  beam:{travel:-.045,spread:.025,rise:-.018,twist:.012},trap:{travel:-.04,spread:.025,twist:.02}
};
...
function animatedAtlasPlane(path,columns,rows,frameCell,opacity=1,grade=null){
  ...
  const material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,side:THREE.DoubleSide,vertexShader:`
    ...
    void main(){
      ...
      p.x+=uRelease*uReleaseDirection*(.025+.028*shoulders);p.y-=uRelease*hem*.022;
      p.x+=uHit*uHitDirection*(uv.y-.22)*.035;
    ...
    void main(){
      ...
      painted.rgb=mix(painted.rgb,vec3(.78,.28,.2)+painted.rgb*.38,uHit*(.07+.13*gradeMask));
      painted.rgb=clamp(painted.rgb+vec3(.07,.035,.018)*uRelease*(.3+.7*vUv.y),0.,1.);
...
export function createHeroVisual(unit){
  ...
  group.add(oathLight,selectedSigil);group.userData={hp,ap,shield,shadow,selectedWash,selectedBracket,oathLight,selectedSigil,sigilOuter,sigilInner,riteHalo,orbitKnives,body,sprite,phase:Math.random()*9,facing:1};prepareDepthSortedVisual(group);return group;
}
...
export function createSkeletonVisual(enemy){
  ...
  group.userData={hp,ap,statusRing,shieldRing,eliteRing,crown,shadow,body,sprite,spriteProfile,attackProfile,phase:Math.random()*8,baseScale,baseOpacity:enemy.template.phase?.72:1,facing:inwardDirection*facingProfile.native};prepareDepthSortedVisual(group);return group;
}

export function createTreasureVisual(){
  const group=new THREE.Group(),shadow=ellipse(.19,.054,flat(0x000000,.26),32),lootGlow=softEllipse(.48,.2,0xe3a64d,.05),sprite=rigidAtlasPlane('/assets/reliquary-shine-atlas.png',2,2),pickTarget=plane(.76,.56,new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthWrite:false,side:THREE.DoubleSide}));
  shadow.position.set(0,-.14,-.12);lootGlow.position.set(0,-.04,-.08);sprite.scale.set(.7,.525,1);sprite.position.set(0,.008,.42);pickTarget.position.set(0,.008,.76);group.add(shadow,lootGlow,sprite,pickTarget);group.userData={shadow,lootGlow,sprite,pickTarget};return group;
}

export function updateTreasureVisual(treasure,time=0){
  const group=treasure?.mesh;if(!group?.userData?.sprite)return;const state=group.userData,sequence=[0,1,2,3,2,1],frame=sequence[Math.floor(time*.0032+treasure.id*.83)%sequence.length],registration=[[-.04,.007],[.04,.007],[-.038,-.007],[.038,-.007]][frame],shine=[.045,.07,.12,.082][frame];group.position.set(treasure.x,treasure.y,.3);group.scale.setScalar(1);group.renderOrder=-12000-Math.round(treasure.y*100);state.sprite.userData.setAnimationFrame(frame);state.sprite.position.set(registration[0],.008+registration[1],.42);state.sprite.material.uniforms.uTint.value.set(treasure.hovered?0xffe8c2:treasure.state==='claimed'?0xffd7a0:0xffffff);state.lootGlow.userData.radialUniforms.uOpacity.value=shine+(treasure.hovered?.035:treasure.state==='claimed'?.018:0);state.shadow.material.opacity=treasure.hovered?.34:.24;
}
...
export function updateEntityVisual(entity,selected=false,time=0){
  ...
  const attackTotal=entity.attackWindupDuration||entity.attackWindup||1,windupProgress=entity.kind==='enemy'&&entity.attackWindup>0?THREE.MathUtils.clamp(1-entity.attackWindup/attackTotal,0,1):0,recoveryTotal=entity.attackVisualRecoveryDuration||1,recovery=entity.kind==='enemy'?THREE.MathUtils.clamp((entity.attackVisualRecovery||0)/recoveryTotal,0,1):0,hitPulse=THREE.MathUtils.clamp(entity.visualHitPulse||0,0,1),action=entity.kind==='hero'?entity.visualCastPulse||0:entity.attackWindup>0?.28+windupProgress*.72:recovery*.72,gaitFrames=[1,0,2,0],frame=action>.16?3:motion>.075?gaitFrames[Math.floor(cadence)%gaitFrames.length]:0;sprite?.userData?.setAnimationFrame?.(frame);const spriteProfile=entity.mesh.userData.spriteProfile,registration=spriteProfile?.frames?.[frame];if(registration)sprite.position.set(registration[0]*spriteProfile.scale,spriteProfile.anchorY+registration[1]*spriteProfile.scale,.14);
  if(spriteUniforms){spriteUniforms.uTime.value=time*.001;spriteUniforms.uMotion.value=motion;spriteUniforms.uAction.value=action;spriteUniforms.uRelease.value=recovery;spriteUniforms.uReleaseDirection.value=entity.kind==='enemy'?(facingProfile?.native||1):1;spriteUniforms.uHit.value=hitPulse;spriteUniforms.uHitDirection.value=(entity.visualHitDirection||0)*facing;spriteUniforms.uStride.value=stride;spriteUniforms.uLean.value=lean}
  if(entity.kind==='hero'){
    const apRatio=Math.max(0,entity.ap/entity.maxAp),apBar=entity.mesh.userData.ap,body=entity.mesh.userData.body,rawCategory=entity.lastAbilityCategory||'melee',castCategory=rawCategory==='nova'?'zone':rawCategory,castProfile=heroCastProfiles[castCategory]||heroCastProfiles.ranged,castOverride=heroCastKindOverrides[entity.lastAbilityKind]||{},castBeat=THREE.MathUtils.smoothstep(action,0,1),castTravel=(castProfile.travel+(castOverride.travel||0))*castBeat,castLift=(castProfile.lift+(castOverride.lift||0))*castBeat,castSpread=(castProfile.spread+(castOverride.spread||0))*castBeat,castRise=(castProfile.rise+(castOverride.rise||0))*castBeat,castTwist=(castProfile.twist+(castOverride.twist||0))*castBeat;setBarValue(apBar,apRatio);hpBar.position.y=selected?-.78:-.78;apBar.position.y=selected?-.92:-.9;hpBar.scale.set(selected?1.04:1,selected?1.08:1,1);apBar.scale.set(selected?1.04:1,selected?1.08:1,1);setBarColors(hpBar,selected?0x6bb57f:0x4f9d67,selected?0x1d1711:0x050609);setBarColors(apBar,selected?0x98a3d2:0x7e89bd,selected?0x1d1711:0x050609);entity.mesh.userData.selectedWash.material.opacity=selected?.13:0;entity.mesh.userData.selectedBracket.material.opacity=selected?.78+Math.sin(time*.004+phase)*.1:0;body.position.x=facing*castTravel+(entity.visualHitDirection||0)*hitPulse*.085;body.position.y=castLift+hitPulse*.018;body.scale.x=facing*(1+motion*Math.cos(cadence*Math.PI/2)*.018+castSpread-hitPulse*.035);body.scale.y=1+action*.018+castRise-motion*Math.abs(stride)*.012+hitPulse*.025;body.rotation.z=-lean*facing*.028+stride*motion*.008+facing*castTwist-(entity.visualHitDirection||0)*hitPulse*.04;
    entity.mesh.position.y+=Math.sin(time*.0022+phase)*.012+Math.abs(stride)*motion*.026;entity.mesh.position.x+=Math.cos(cadence*Math.PI/2)*motion*.006;entity.mesh.userData.shield.material.opacity=entity.shield>0?.18+Math.sin(time*.004)*.07:0;entity.mesh.userData.shield.rotation.z+=.006;const oathLight=entity.mesh.userData.oathLight,selectedSigil=entity.mesh.userData.selectedSigil,sigilOuter=entity.mesh.userData.sigilOuter,sigilInner=entity.mesh.userData.sigilInner,selectionBreath=.5+.5*Math.sin(time*.0028+phase),castHalo=castProfile.halo||.12;if(oathLight){oathLight.userData.radialUniforms.uOpacity.value=selected?.18+selectionBreath*.045:.068+selectionBreath*.012+Math.min(.05,action*(.018+castHalo*.1));const lightScale=1+action*(.12+castHalo*.28)+selectionBreath*(selected?.045:.012);oathLight.scale.set(lightScale,lightScale,1)}if(selectedSigil){selectedSigil.visible=selected;if(selected){selectedSigil.rotation.z=time*.0002+Math.sin(time*.0007+phase)*.045;sigilOuter.material.opacity=.48+selectionBreath*.2;sigilInner.material.opacity=.2+selectionBreath*.12;selectedSigil.children.slice(2).forEach((tick,index)=>{tick.material.opacity=(index%2?.38:.65)+selectionBreath*.14})}}
    ...
    const birthDuration=entity.visualBirthDuration??.68,birthAge=entity.visualBirthAge??birthDuration,birthT=Math.min(1,birthAge/birthDuration),reveal=1-Math.pow(1-birthT,3),baseScale=entity.mesh.userData.baseScale||1;entity.mesh.scale.setScalar(baseScale*(.7+reveal*.3));if(spriteUniforms)spriteUniforms.uOpacity.value=(entity.mesh.userData.baseOpacity||1)*(.08+reveal*.92);
    const winding=entity.attackWindup>0,body=entity.mesh.userData.body,attackProfile=entity.mesh.userData.attackProfile||enemyAttackProfiles.thrall,gather=THREE.MathUtils.smoothstep(windupProgress,0,1),releaseKick=recovery*recovery,worldFacing=facing*(facingProfile?.native||1),hitDirection=entity.visualHitDirection||0;body.position.x=worldFacing*(-attackProfile.pull*gather+attackProfile.lunge*releaseKick)+hitDirection*hitPulse*.085;body.position.y=-(1-reveal)*.28-attackProfile.crouch*gather+Math.abs(attackProfile.crouch)*releaseKick*.3+hitPulse*.014;body.scale.x=facing*(1+motion*Math.cos(cadence*Math.PI/2)*.014+releaseKick*.035-hitPulse*.04);body.scale.y=1+(winding?.025:0)-motion*Math.abs(stride)*.01-attackProfile.crouch*gather*.22+releaseKick*.045+hitPulse*.028;body.rotation.z=-lean*facing*.022+stride*motion*.006+worldFacing*attackProfile.twist*(gather-releaseKick*.72)-hitDirection*hitPulse*.04;entity.mesh.position.y+=Math.abs(stride)*Math.min(.034,speed*.024);entity.mesh.position.x+=Math.cos(cadence*Math.PI/2)*motion*.004;const apBar=entity.mesh.userData.ap;if(apBar){const apRatio=Math.max(0,entity.ap/entity.maxAp);setBarValue(apBar,apRatio);setBarColors(apBar,winding?0xd05b52:0x737eaa,0x16171b)}
    const status=entity.status;const ring=entity.mesh.userData.statusRing,statusActive=status.burn>0||status.brittle>0||status.curse>0;ring.visible=statusActive;if(status.burn>0){ring.material.color.set(0xc75c3e);ring.material.opacity=.42}else if(status.brittle>0){ring.material.color.set(0x8faac3);ring.material.opacity=.38}else if(status.curse>0){ring.material.color.set(0x9a5ca7);ring.material.opacity=.4}else ring.material.opacity=0;if(statusActive)ring.rotation.z+=.01;const eliteRing=entity.mesh.userData.eliteRing;if(eliteRing){eliteRing.visible=Boolean(entity.elite);if(eliteRing.visible){eliteRing.rotation.z+=.008;eliteRing.material.opacity=.34+Math.sin(time*.004+entity.mesh.userData.phase)*.14}}const shieldRing=entity.mesh.userData.shieldRing;if(shieldRing){shieldRing.visible=entity.shield>0;if(shieldRing.visible){shieldRing.material.opacity=.2+Math.sin(time*.006)*.1;shieldRing.rotation.z-=.012}else shieldRing.material.opacity=0}if(entity.mesh.userData.crown)entity.mesh.userData.crown.position.y=Math.sin(time*.004+entity.mesh.userData.phase)*.035;
  }
}
...
export function createEnemyIntent(scene,enemy,target,color,duration,kind='melee'){
  const ranged=kind==='ranged',cleave=kind==='cleave',kindIndex=cleave?2:ranged?1:0,strikeRadius=cleave?enemy.range+1:0,uniforms={uColor:{value:new THREE.Color(color)},uSize:{value:new THREE.Vector2(1,1)},uSpan:{value:1},uRadius:{value:strikeRadius},uProgress:{value:0},uKind:{value:kindIndex},uAngle:{value:0},uSeed:{value:(enemy.id%17)*.73+kindIndex*2.1}},material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,depthTest:true,side:THREE.DoubleSide,vertexShader:`
    varying vec2 vUv;
  ...
  `,fragmentShader:`
    ...
    uniform float uSeed;
    float segmentDistance(vec2 p,vec2 a,vec2 b){vec2 pa=p-a,ba=b-a;float h=clamp(dot(pa,ba)/max(.0001,dot(ba,ba)),0.,1.);return length(pa-ba*h);}
    float stroke(float distanceValue,float width){float aa=max(fwidth(distanceValue)*1.25,.0015);return 1.-smoothstep(width,width+aa,abs(distanceValue));}
    float ellipseBand(vec2 p,vec2 radius,float width){return stroke(length(p/max(radius,vec2(.001)))-1.,width);}
    float lineStroke(vec2 p,vec2 a,vec2 b,float width){return stroke(segmentDistance(p,a,b),width);}
    void main(){
      vec2 p=(vUv-.5)*uSize,source=vec2(-uSpan*.5,0.),target=vec2(uSpan*.5,0.);float finale=smoothstep(.7,1.,uProgress),ink=.78+.22*sin(p.x*17.+p.y*23.+uSeed)*sin(p.x*11.-p.y*13.+uSeed*1.7),alpha=0.,soft=0.;
      if(uKind<.5){
        vec2 from=p-source,to=p-target;float tether=lineStroke(p,source+vec2(.35,0.),target-vec2(.14,0.),.012),dashes=.34+.66*step(.44,fract((p.x-source.x)*2.8-uProgress*.55));float crescent=ellipseBand(from,vec2(.5,.36),.036)*smoothstep(-.18,.2,from.x);float gather=ellipseBand(from,vec2(.57,.42)*(1.08-uProgress*.16),.018);float bracket=max(lineStroke(to,vec2(0.,-.24),vec2(0.,.24),.018),max(lineStroke(to,vec2(0.,-.24),vec2(.17,-.24),.018),lineStroke(to,vec2(0.,.24),vec2(.17,.24),.018)));float slash=max(lineStroke(to,vec2(-.18,-.22),vec2(.2,.22),.014),lineStroke(to,vec2(-.12,.2),vec2(.16,-.16),.009));alpha=max(alpha,tether*dashes*(.29+finale*.39));alpha=max(alpha,crescent*(.42+finale*.52));alpha=max(alpha,gather*(.17+finale*.21));alpha=max(alpha,bracket*(.3+finale*.5));alpha=max(alpha,slash*finale*.72);soft=max(soft,ellipseBand(from,vec2(.5,.36),.11)*(.06+finale*.055));
      }else if(uKind<1.5){
        vec2 from=p-source,to=p-target;float thread=lineStroke(p,source+vec2(.27,0.),target-vec2(.3,0.),.01),vein=lineStroke(p,source+vec2(.25,0.),target-vec2(.28,0.),.055),dashes=.28+.72*step(.38,fract((p.x-source.x)*4.2-uProgress*1.45));float sourceSeal=ellipseBand(from,vec2(.29,.22),.04);float closing=mix(.58,.38,uProgress),targetSeal=max(ellipseBand(to,vec2(closing,closing*.76),.032),stroke(abs(to.x)+abs(to.y/.76)-closing*.72,.022));float cross=max(lineStroke(to,vec2(-.24,0.),vec2(.24,0.),.012),lineStroke(to,vec2(0.,-.18),vec2(0.,.18),.012));float traveller=exp(-pow((p.x-mix(source.x,target.x,uProgress))/max(.035,uSpan*.018),2.))*exp(-abs(p.y)*18.);alpha=max(alpha,thread*dashes*(.35+finale*.55));alpha=max(alpha,vein*(.05+finale*.07));alpha=max(alpha,sourceSeal*(.32+finale*.43));alpha=max(alpha,targetSeal*(.48+finale*.48));alpha=max(alpha,cross*(.2+finale*.64));alpha=max(alpha,traveller*(.24+finale*.56));soft=max(soft,ellipseBand(to,vec2(closing,closing*.76),.13)*(.075+finale*.075));
      }else{
        vec2 ep=vec2(p.x,p.y/.76);float radius=length(ep),angle=fract((atan(ep.y,ep.x)+3.14159265)/6.2831853),segments=.36+.64*step(.22,fract(angle*12.+uSeed*.17)),sweep=step(angle,.12+uProgress*.88);float outer=stroke(radius-uRadius,.026),inner=stroke(radius-uRadius*.79,.018),closing=stroke(radius-uRadius*(1.09-uProgress*.09),.012),ticks=stroke(radius-uRadius*.9,.055)*smoothstep(.7,.96,abs(sin(angle*3.14159265*16.)));vec2 aimDirection=normalize(vec2(cos(uAngle),sin(uAngle)/.76));float ca=aimDirection.x,sa=aimDirection.y;vec2 aim=vec2(ca*ep.x+sa*ep.y,-sa*ep.x+ca*ep.y);float spoke=lineStroke(aim,vec2(uRadius*.24,0.),vec2(uRadius*.92,0.),.014),fang=max(lineStroke(aim,vec2(uRadius*.82,0.),vec2(uRadius*.65,.16*uRadius),.014),lineStroke(aim,vec2(uRadius*.82,0.),vec2(uRadius*.65,-.16*uRadius),.014));alpha=max(alpha,outer*segments*(.32+finale*.56));alpha=max(alpha,inner*sweep*(.2+finale*.3));alpha=max(alpha,closing*(.14+finale*.2));alpha=max(alpha,ticks*(.13+finale*.29));alpha=max(alpha,spoke*(.2+finale*.5));alpha=max(alpha,fang*finale*.7);soft=max(soft,stroke(radius-uRadius,.12)*(.06+finale*.06));
      }
      alpha=clamp((alpha*ink+soft)*(.78+finale*.22),0.,.96);if(alpha<.008)discard;vec3 chalk=mix(uColor,vec3(.86,.81,.72),.08+finale*.18);gl_FragColor=linearToOutputTexel(vec4(chalk,alpha));
    }
  `}),quad=plane(1,1,material);quad.position.z=.045;quad.renderOrder=-15000;scene.add(quad);let age=0,cancelled=false;
  const place=()=>{const dx=target.x-enemy.x,dy=target.y-enemy.y,length=Math.max(.1,Math.hypot(dx,dy)),angle=Math.atan2(dy,dx);uniforms.uAngle.value=angle;if(cleave){const extent=strikeRadius*2.32;quad.position.set(enemy.x,enemy.y,.045);quad.scale.set(extent,extent,1);quad.rotation.z=0;uniforms.uSize.value.set(extent,extent);uniforms.uSpan.value=0}else{const height=ranged?1.45:1.18,width=length+(ranged?1.5:1.3);quad.position.set((enemy.x+target.x)/2,(enemy.y+target.y)/2,.045);quad.scale.set(width,height,1);quad.rotation.z=angle;uniforms.uSize.value.set(width,height);uniforms.uSpan.value=length}};place();return{mesh:quad,drawCalls:1,kind,cancel(){cancelled=true},update(dt){if(cancelled)return true;age+=dt;uniforms.uProgress.value=Math.min(1,age/duration);place();return age>=duration+.06},destroy(){scene.remove(quad);disposeObject(quad)}};
}

export function createCorpseDecal(scene,x,y,type){
  const group=new THREE.Group(),sprite=rigidAtlasPlane('/assets/undead-remains-atlas.png',4,3),index=Math.max(0,enemyArtOrder.indexOf(type)),large=type==='giant'||type==='ossuary',small=type==='hound',baseX=large?2.55:small?1.45:1.85,baseY=large?1.92:small?1.08:1.39,settledY=small?.86:1,stain=ellipse((large?1.05:small?.58:.78),(large?.35:small?.18:.26),flat(0x050507,0),36),collapse=animatedEffect('impact',0xb8aa95,.38),dustGroup=new THREE.Group(),splinters=[],splinterCount=large?6:small?3:4;let transientsDisposed=false;
  sprite.userData.setAnimationFrame(index);sprite.userData.setOpacity(0);sprite.material.uniforms.uTint.value.setRGB(.68,.64,.58);sprite.material.uniforms.uLift.value=-.018;sprite.scale.set(baseX*.82,baseY*.5,1);sprite.position.y=.14;stain.position.z=-.04;collapse.scale.set(large?2.15:small?1.1:1.55,large?1.45:small?.72:1.02,1);collapse.position.z=.06;stain.renderOrder=sprite.renderOrder=-20000;collapse.renderOrder=-9000;
  ...
  return{kind:'corpse',age:0,duration:12,prominent:true,prominence:1,forcedFade:null,setProminent(value){this.prominent=value},forceFade(){if(this.forcedFade==null)this.forcedFade=0},update(dt){this.age+=dt;if(this.forcedFade!=null)this.forcedFade+=dt;this.prominence=THREE.MathUtils.lerp(this.prominence,this.prominent?1:0,Math.min(1,dt*4.5));const settle=Math.min(1,this.age/.32),ease=1-Math.pow(1-settle,3),naturalFade=this.age<=9?1:Math.max(0,1-(this.age-9)/3),capacityFade=this.forcedFade==null?1:Math.max(0,1-this.forcedFade/.55),opacity=ease*(.68+this.prominence*.32)*naturalFade*capacityFade;sprite.userData.setOpacity(opacity);sprite.scale.set(baseX*(.82+ease*.18),baseY*(.5+ease*(settledY-.5)),1);sprite.position.y=.14*(1-ease);stain.material.opacity=.075*ease*naturalFade*capacityFade;stain.scale.setScalar(.7+ease*.3);if(this.age<.64){const dustT=Math.min(1,this.age/.62);animateEffect(collapse,Math.min(.999,this.age/.48),.38*(1-dustT));collapse.scale.multiplyScalar(1+dt*.42);splinters.forEach((mark,shard)=>{const travel=dustT*(large?.42:small?.2:.3);mark.position.x=mark.userData.originX+Math.cos(mark.userData.angle)*travel;mark.position.y=mark.userData.originY+Math.sin(mark.userData.angle)*travel+dustT*.16;mark.rotation.z+=dt*(shard%2?4.4:-3.8);mark.material.opacity=.66*(1-dustT)})}else if(!transientsDisposed){group.remove(collapse,dustGroup);disposeObject(collapse);disposeObject(dustGroup);transientsDisposed=true}return this.age>=this.duration||this.forcedFade>=.55},destroy(){scene.remove(group);disposeObject(group)}}
}
