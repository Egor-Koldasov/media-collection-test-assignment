...
function makeBar(width,color){
  const uniforms={uFill:{value:1},uFillColor:{value:new THREE.Color(color)},uBgColor:{value:new THREE.Color(0x16171b)}},material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,toneMapped:false,vertexShader:`varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,fragmentShader:`
    ...
    void main(){
      vec3 color=vec3(.019,.022,.03);float alpha=.84;
      float frame=max(max(step(vUv.x,.035),step(.965,vUv.x)),max(step(vUv.y,.1),step(.9,vUv.y)));
      color=mix(color,vec3(.604,.545,.463),frame*.28);
      float innerX=step(.045,vUv.x)*step(vUv.x,.955),innerY=step(.2,vUv.y)*step(vUv.y,.8),inside=innerX*innerY;
      color=mix(color,uBgColor,inside);alpha=mix(alpha,.92,inside);
      float fillEdge=.045+uFill*.91,filled=inside*step(vUv.x,fillEdge);
      color=mix(color,uFillColor,filled);alpha=mix(alpha,1.,filled);
      float gleam=filled*step(.61,vUv.y)*step(vUv.y,.69);
      color=mix(color,vec3(.937,.894,.82),gleam*.16);
      gl_FragColor=vec4(color,alpha);
    }`});
  const bar=plane(width+.09,.086,material);bar.userData.barUniforms=uniforms;return bar;
}
...
export function updateEntityVisual(entity,selected=false,time=0){
  ...
  if(entity.kind==='hero'){
    ...
    const birthAge=Number.isFinite(entity.visualBirthTime)?Math.max(0,(time-entity.visualBirthTime)/1000):1,birthT=Math.min(1,birthAge/.68),reveal=1-Math.pow(1-birthT,3),baseScale=entity.mesh.userData.baseScale||1;entity.mesh.scale.setScalar(baseScale*(.7+reveal*.3));if(spriteUniforms)spriteUniforms.uOpacity.value=(entity.mesh.userData.baseOpacity||1)*(.08+reveal*.92);entity.mesh.userData.body.position.y=-(1-reveal)*.28;
