...
export function buildArena(scene){
  ...
  const phaseAura=ellipseLoop(8.72,6.56,0x8b6b52,0,128);phaseAura.position.z=-1.3;scene.add(phaseAura);const phaseIris=ellipseLoop(6.15,4.62,0x8b6b52,0,96);phaseIris.position.z=-1.27;scene.add(phaseIris);const phaseWave=ellipseLoop(1.12,.84,0x8b6b52,0,128),phaseVeil=softEllipse(7.8,5.7,0x8b6b52,0);phaseWave.position.z=-1.16;phaseVeil.position.z=-1.7;scene.add(phaseVeil,phaseWave);
  ...
  const lawSeals=new THREE.Group();scene.add(lawSeals);let pulseIntensity=0,activeLawColors=[],bellPhase=0,phaseFlash=0,phaseWaveAge=1;const phaseColors=[0x8b6b52,0x8b6b52,0x675775,0x8f3440,0xc17953];scene.traverse((object)=>{object.renderOrder=-30000});
  return{
    pulse(){pulseIntensity=1},
    setBell(bell){const next=bell>=18?4:bell>=15?3:bell>=10?2:bell>=5?1:0;if(next!==bellPhase){bellPhase=next;phaseFlash=1;phaseWaveAge=0;phaseAura.material.color.set(phaseColors[bellPhase]);phaseIris.material.color.set(phaseColors[bellPhase]);phaseWave.material.color.set(phaseColors[bellPhase]);phaseVeil.userData.radialUniforms.uColor.value.set(phaseColors[bellPhase]);sigilGlow.material.color.set(phaseColors[bellPhase]);sigil.visible=bellPhase>=2;eclipseHalo.visible=eclipse.visible=eclipseRim.visible=bellPhase>=3;pulseIntensity=1}},
    applyLaws(laws=[]){
    ...
    update(time,progress=0){
      ...
      phaseFlash*=.955;phaseWaveAge=Math.min(1,phaseWaveAge+.012);const waveEase=1-Math.pow(1-phaseWaveAge,3),waveFade=Math.sin(phaseWaveAge*Math.PI);phaseWave.scale.setScalar(.85+waveEase*8.2);phaseWave.material.opacity=bellPhase?waveFade*(.2+bellPhase*.045):0;phaseVeil.userData.radialUniforms.uOpacity.value=bellPhase*.009+phaseFlash*.09;innerStain.material.color.lerp(new THREE.Color(phaseColors[bellPhase]),bellPhase?.004:0);phaseAura.material.opacity=bellPhase ? .025+bellPhase*.014+phaseFlash*.24 : 0;phaseIris.material.opacity=bellPhase ? .018+bellPhase*.012+phaseFlash*.16 : 0;phaseAura.rotation.z=time*(bellPhase%2?-.000035:.00004);phaseIris.rotation.z=time*.00007;const phaseBreath=1+Math.sin(time*.0008)*(.006+bellPhase*.002)+phaseFlash*.045;phaseAura.scale.set(phaseBreath,phaseBreath,1);phaseIris.scale.set(2-phaseBreath,2-phaseBreath,1);runeGroup.rotation.z=Math.sin(time*.00008)*.008+bellPhase*time*.0000025;runeGroup.children.forEach((rune,index)=>{rune.userData.mark.material.opacity=bellPhase?Math.min(.28,.1+bellPhase*.035+Math.sin(time*.0011+index)*.035):0});
...
function makeBar(width,color){
  const uniforms={uFill:{value:1},uFillColor:{value:new THREE.Color(color)},uBgColor:{value:new THREE.Color(0x16171b)}},material=new THREE.ShaderMaterial({uniforms,transparent:true,depthWrite:false,toneMapped:false,vertexShader:`varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,fragmentShader:`
    ...
    void main(){
      vec3 color=vec3(.019,.022,.03);float alpha=.84;float aa=max(fwidth(vUv.y)*.72,.002);
      float frameInner=smoothstep(.035,.035+aa,vUv.x)*smoothstep(.035,.035+aa,1.-vUv.x)*smoothstep(.1,.1+aa,vUv.y)*smoothstep(.1,.1+aa,1.-vUv.y),frame=1.-frameInner;
      color=mix(color,vec3(.604,.545,.463),frame*.28);
      float innerX=smoothstep(.045,.045+aa,vUv.x)*smoothstep(.045,.045+aa,1.-vUv.x),innerY=smoothstep(.2,.2+aa,vUv.y)*smoothstep(.2,.2+aa,1.-vUv.y),inside=innerX*innerY;
      color=mix(color,uBgColor,inside);alpha=mix(alpha,.92,inside);
      float fillEdge=.045+uFill*.91,filled=inside*(1.-smoothstep(fillEdge-aa,fillEdge+aa,vUv.x))*step(.0001,uFill);
      color=mix(color,uFillColor,filled);alpha=mix(alpha,1.,filled);
      float gleam=filled*smoothstep(.61,.61+aa,vUv.y)*smoothstep(.31,.31+aa,1.-vUv.y);
...
export function createEnemyIntent(scene,enemy,target,color,duration,kind='melee'){
  ...
  else if(cleave){sourceMark=new THREE.Mesh(new THREE.RingGeometry(.86,1.01,52,1,-.18,Math.PI*1.34),flat(color,.24));targetMark=new THREE.Mesh(new THREE.RingGeometry(1.16,1.24,52,1,Math.PI*1.48,Math.PI*.36),flat(0xc9bda8,.14));accent=line([[-1.02,-.14,0],[-.79,-.38,0],[-.5,-.48,0]],color,.2);group.add(sourceMark,targetMark,accent)}
  else{sourceMark=new THREE.Mesh(new THREE.RingGeometry(.43,.54,40,1,-.72,Math.PI*1.42),flat(color,.26));targetMark=line([[.2,-.34,0],[.43,-.18,0],[.55,.08,0]],0xd9cdbb,.16);accent=line([[-.06,.48,0],[.17,.32,0]],color,.22);group.add(sourceMark,targetMark,accent)}
  group.position.z=.36;scene.add(group);let age=0,cancelled=false;return{cancel(){cancelled=true},update(dt){if(cancelled)return true;age+=dt;const dx=target.x-enemy.x,dy=target.y-enemy.y,length=Math.max(.1,Math.hypot(dx,dy)),t=Math.min(1,age/duration),finale=THREE.MathUtils.smoothstep(t,.78,1),pulse=.82+Math.sin(t*Math.PI*5)*.08*finale;group.position.x=enemy.x;group.position.y=enemy.y;group.rotation.z=Math.atan2(dy,dx);if(ranged){beam.scale.x=glow.scale.x=length;beam.position.x=glow.position.x=length/2;targetMark.position.x=length;targetMark.scale.setScalar((1.28-t*.58)*pulse);sourceMark.scale.setScalar(.85+t*.15);beam.material.opacity=.09+finale*.62;glow.material.opacity=.018+finale*.16;sourceMark.material.opacity=.16+finale*.55;targetMark.material.opacity=.27+finale*.66;accent.material.opacity=.12+finale*.74;targetMark.rotation.z-=dt*(1.1+finale*5)}else if(cleave){const gather=.9-t*.16;sourceMark.scale.setScalar(gather);targetMark.scale.setScalar(gather);sourceMark.material.opacity=.18+finale*.68;targetMark.material.opacity=.1+finale*.52;accent.material.opacity=.14+finale*.7;sourceMark.rotation.z+=dt*(.35+finale*2.5);targetMark.rotation.z-=dt*(.25+finale*1.7)}else{const gather=1.08-t*.22;sourceMark.scale.setScalar(gather);sourceMark.material.opacity=.18+finale*.72;targetMark.material.opacity=.1+finale*.68;accent.material.opacity=.14+finale*.74;sourceMark.rotation.z+=dt*(.45+finale*3.6)}return age>=duration+.06},destroy(){scene.remove(group);disposeObject(group)}};
}
